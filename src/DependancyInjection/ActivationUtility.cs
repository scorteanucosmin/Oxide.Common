...
namespace Oxide.DependencyInjection
{
    ...
    {
        private static readonly IClaimable<ParameterMap> mapPool = Pool.Provider.GetPool<ParameterMap>();
        private static readonly IArrayPool<int?> positionsPool = (IArrayPool<int?>)Pool.Provider.GetPool<int?[]>();
        private static readonly IArrayPool<object> argumentPool = (IArrayPool<object>)Pool.Provider.GetPool<object[]>();
        ...
        internal class ParameterMap : IDisposable, IResetable
        {
            ...
            public bool Init(MemberInfo member)
            {
                ...
                Arguments = argumentPool.Claim(Parameters.Length);
                PositionMap = positionsPool.Claim(Parameters.Length);
            ...
            public void Reset()
            {
                ...
                Arguments = null;
                argumentPool.Unclaim(ref args);
                int?[] map = PositionMap;
                PositionMap = null;
                positionsPool.Unclaim(ref map);
                Mapped = 0;
                Found = 0;
            }
        ...
        internal static ConstructorInfo FindSuitableConstructor(ConstructorInfo[] constructors, object[] parameters, IServiceProvider provider, out ParameterMap map)
        {
            ...
            for (int i = 0; i < constructors.Length; i++)
            {
                ...
                ParameterMap currMap = mapPool.Claim();
                currMap.Init(con);
        ...
        internal static object CreateInstance(ConstructorInfo constructor, IServiceProvider provider, object[] parameters)
        {
            ...
            ParameterMap map = mapPool.Claim();
